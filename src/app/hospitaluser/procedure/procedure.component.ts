...
export class ProcedureComponent implements OnInit {
  ...
  ngOnInit() {
    const procedureListString:string|null=localStorage.getItem("procedureData");
    if(procedureListString)
      this.procedureData = JSON.parse(procedureListString);
    ...
    if (this.procedureForm.valid && this.newProcedureForm.valid) {
      ...
      if (this.codeControl.value == 'Not Defined') {
        ...
        if(found==-1){
          ...
          this.toastr.error("Procedure Already Added");
          return;
        }    
      }
  ...
  submit_click() {
    if (this.selectedProcedure.length > 0) {
      ...
      this.toastr.error("Please add suitable procedure");
    }
  }
