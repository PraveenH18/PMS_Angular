...
export class DiagnosisComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,private dbService:HospitaluserService,private toastr:ToastrService) {
  }
  ...
  ngOnInit(): void {
    ...
    //this.diagnosisData = this.route.snapshot.data['diagnosisData'];
    const diagnosisListString:string|null=localStorage.getItem("diagnosisData");
    if(diagnosisListString)
      this.diagnosisData = JSON.parse(diagnosisListString);
     // this.diagnosisData.push(new Diagnosis("Not Defined","Other Diagnosis",false));
  ...
  submit_click() {
    if(this.selectedDiagnosis.length>0){
      ...
      this.dbService.saveDiagnosisDetails(diagnosisDto).subscribe(
        ...
        (error)=>{
          console.log(error);
          this.toastr.error("Error Occured While Saving Diagnosis Details");
        }
      );
     
    }else{
      this.toastr.error("Please add suitable diagnosis");
    }
  }
