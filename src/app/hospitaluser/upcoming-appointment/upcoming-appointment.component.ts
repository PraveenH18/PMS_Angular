...
export class UpcomingAppointmentComponent implements OnInit {
  ...
  constructor(private toastr: ToastrService,
    private schddulerService: AppointmentService, private datepipe: DatePipe,
  ...
  ngOnInit(): void {
    if (this.userRole == 'nurse') {
      ...
      const nurseStr: string | null = sessionStorage.getItem("nurse");
      if (nurseStr) {
        const nurse = JSON.parse(nurseStr);
        this.creatorName = nurse.title + " " + nurse.firstName + " " + nurse.lastName+"(Staff)";
    ...
    if (this.userRole == 'physician') {
      ...
      const physicianStr: string | null = sessionStorage.getItem("physician");
      if (physicianStr) {
        const physician = JSON.parse(physicianStr);
        this.creatorName = physician.employee.title + " " + physician.employee.firstName + " " + physician.employee.lastName +"(Doctor)";
        this.specialization.setValue(physician.speciality.name);
        this.physicianName.setValue(physician.employee.title + " " + physician.employee.firstName + " " + physician.employee.lastName);
        this.physicianId.setValue(physician.employee.user.userId);
  ...
  public updateAppointment(element:any){
    ...
    if (endTime != null && startTime != null) {
      ...
      updateAppointment.patientId = element.PatientId;
  ...
  if(this.userRole=='physician'){
    ...
    if (endTime != null && startTime != null) {
      ...
      appointment.patientId = element[0].PatientId;
